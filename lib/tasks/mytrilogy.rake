...
namespace :generate do
  ...
  task :migration, [:model_name, :down] => :environment do |task, options|
    erb_string = ERB.new(File.read("#{File.dirname(__FILE__)}/../mytrilogy/templates/migration_template.erb")).result(binding)
    migrate_directory = "#{Rails.root}/db/migrate"
    time_stamp = Time.now.utc.strftime("%Y%m%d%H%M%S")
    file_path = File.join(migrate_directory, "#{time_stamp}_#{options[:model_name].underscore}.rb")
